...
An iniciative of [BrazilJS Foundation](http://braziljs.org) to help those people who want to organize conferences or events and don't have too much time to create the website of it.
...
We use [DocPad](https://github.com/bevry/docpad), a static generator for Node.js, to create an easily customizable template. More than that, hosting is free via [GitHub Pages](http://pages.github.com) and you can use your own domain *(more information about that on [Deploy](#custom-domain))*
...
1. Install [Git](http://git-scm.com/downloads) and [Node.js](http://nodejs.org/download/), if you don't have it yet.
...
This is where the generated files are stored, once DocPad has been runned. However, this directory is not required in versioning, so it is ignored ([.gitignore](https://github.com/braziljs/conf-boilerplate/blob/master/.gitignore)).
...
List Node.js modules dependencies.

### [publish.sh](https://github.com/braziljs/conf-boilerplate/blob/master/publish.sh)

Shell Script responsible for publishing the site via [GitHub Pages](http://pages.github.com).

## Customization

The project already comes with a visual template, feel free to use it, but we recommend you to create your own in order to put your own identity in the event.
...
Still don't get a full schedule of the event? No problem, just comment out the `schedule` line (using `#`).

Still don't know who is going to speak? Ok, just comment out the `speakers` line (using `#`).
...
You can also change the order in which they will be shown on the page and navigation by changing the order of lines here!

### Labels (i18n)

If you want to use different words than default or a different language
just change the labels for the corresponding elements:
...
To change the time of check-in, lunch and coffee break, just see the `schedule` variable.
...
But if you want to add another coffee break or any kind of item on the agenda, just add the item on the list.

### List of Sponsors/Partners

To add any sponsor or partner, just use the `sponsors` and `partners` variables.
...
1. Create a `CNAME` file under `src/files` folder and fill with your domain: `your-event.com`.
...
The conferences that already used this project as a kickstart:
...
Have you created a website using ConfBoilerplate? Let us know =D
...
Special thanks to all the community members for feedbacks and contributions.
