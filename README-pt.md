...
Nós usamos o [Metalsmith](http://www.metalsmith.io), um static generator em Node, para criar esse modelo extremamente simples de customizar. Além disso, a hospedagem é gratuita via [GitHub Pages](http://pages.github.com) ou você ainda pode usar seu próprio domínio *(mais informações em [Deploy](#domnio-personalizado))*.
...
*OBS 1: Não há integração com nenhum sistema de inscrição e/ou pagamento. Por conta disso, indicamos o [Stripe](https://stripe.com/) ou [Eventick](http://eventick.com.br/).*
...
1. Instale [Git](http://git-scm.com/downloads) e [Node](http://nodejs.org/download/), caso você não os tenha ainda.
...
Responsável por configurar o processo de geração estática.

### [package.json](https://github.com/braziljs/conf-boilerplate/blob/master/package.json)

Lista as dependências de módulos do Node.
...
De qualquer forma, nós preparamos algo altamente customizável para você, portanto para maioria conferenceções do projeto basta ir até o `metalsmith.js` e alterar o valor das variáveis.
...
Ainda não definiu a programação completa do evento? Ainda não sabe quem irá palestrar? Não tem problema, basta remover a chamada ao partial em `src/documents`.
...
Você ainda pode trocar a order em que eles aparecem na navegação do topo ao trocar a order das linhas.
...
Quer listar mais algum atributo do palestrante que não está ali? Tudo bem, é só adicionar no `conference.js` e depois exibí-lo com `<%= speaker.seuNovoAtributo %>` no [speakers.html.eco](https://github.com/braziljs/conf-boilerplate/blob/master/src/partials/section/speakers.html.eco).
...
Agradecimento especial a todos os membros da comunidade pelos feedbacks e [contribuições](https://github.com/braziljs/conf-boilerplate/graphs/contributors).
